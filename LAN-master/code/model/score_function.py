...
class TransE(object):
    ...
    def __call__(self, head, tail, relation):
        ...
        dissimilarity = tf.reduce_sum(input_tensor=tf.abs(head + relation - tail), axis=1)
        score = -dissimilarity
        return score 
...
class Distmult(object):
    ...
    def __call__(self, head, tail, relation):
        score = tf.reduce_sum(input_tensor=head * relation * tail, axis=1)
        return score 
...
class Dotproduct(object):
    ...
    def __call__(self, head, tail, relation):
        score =  tf.reduce_sum(input_tensor=head * tail, axis=1)
        return score
...
class TransL(object):
    ...
    def __call__(self, head, tail, relation):
        head = tf.nn.l2_normalize(head, 1)
        tail = tf.nn.l2_normalize(tail, 1)
        score =  tf.reduce_sum(input_tensor=tf.abs(head - tail), axis=1)
        return -score
...
class Complex(object):
    ...
    def __call__(self, head, tail, relation):
        ...
        score = tf.reduce_sum(input_tensor=h1 * t1 * r1 + h2 * t2 * r1 + h1 * t2 * r2 - h2 * t1 * r2, axis=1)
        return score
...
class Analogy(object):
    ...
    def __call__(self, head, tail, relation):
        ...
        score = tf.reduce_sum(input_tensor=relation[:, 0:offset*2]*head[:, 0:offset*2]*tail[:,0:offset*2], axis=1)
